...
import java.util.*;
public class MyTankGame8_4 extends JFrame implements ActionListener{
...
//	    String p=null;
	...
	public static void main(String[] args){
		MyTankGame8_4 mtk=new MyTankGame8_4();

	}
	
	//构造函数
	public MyTankGame8_4(){
...
//	//定义一个二维的对象数组（存储墙对象）
    ...
    Image image6=null;
	 ...
	 public Mypanel(){
		...
		AePlayWave apw=new AePlayWave("D:/java项目/MyTankGame/src/111.wav");
		apw.start();
...
//		g.drawImage(image6, 100, 60,80,30, this);
		...
		{   
			AePlayWave apw=new AePlayWave("D:/java项目/MyTankGame/src/2643.wav");
			apw.start();
			//设置画笔颜色
		...
		//将所有敌人坦克消灭
		  if(Recorder.getEnNum()==0)
		 {  
			AePlayWave apw=new AePlayWave("D:/java项目/MyTankGame/src/659.wav");
			apw.start();
			//设置画笔颜色
		   ...
		   case 1:
			        if(s.x>(et.x-10)&&s.x<(et.x+10)&&s.y>(et.y-15)&&s.y<(et.y+15))
			       {
				     ...
				     //当敌人坦克被击中时，下面的提示信息数量减一
				      ...
				      //通过线程，让爆炸有音效
				      AePlayWave apw1=new AePlayWave("d:/java项目/MyTankGame/src/1776.wav");
		          	  apw1.start();
				      //创建一颗炸弹
	...
	public void keyPressed(KeyEvent e) {
		...
		if(e.getKeyCode()==KeyEvent.VK_W){
			...
			this.hero.moveUp();
			AePlayWave apw1=new AePlayWave("d:/java项目/MyTankGame/src/1858.wav");
			apw1.start();
			 }
		}//坦克向下
		...
		if(e.getKeyCode()==KeyEvent.VK_J){
			if(hero.ss.size()<=4)
			{
			AePlayWave apw1=new AePlayWave("d:/java项目/MyTankGame/src/2472.wav");
			apw1.start();
			this.hero.shotEnemy();
			 }
	    }
			   ...
			   switch(hero.direction)
		    	  {
		    	   ...
		    	   case 0:
		    		   /*
		    		    * 下面这句话很重要，当我的坦克碰着墙以后，isTouchWall的值
		    		    * 变为true,这时我的坦克无法前进，但之后并没有将isTouchWall的值改为
		    		    * false,所以按任何一个方向，坦克都无法移动，下面这句话的意思就是改
		    		    * isTouchWall的值为false,使我的坦克可以继续移动。（主要意义是
		    		    * 当我的坦克是除了“向上”后的其他方向，而且碰着墙了，这时isTouchWall
		    		    * 值为true,按向上时，就可以改isTouchWall为false,使其可以继续移动）
		    		    */
		    		   Hero.isTouchWall=false;
		    		   for(int j=0;j<4;j++)
		    		   for(int k=0;k<12;k++)
		    ...
		    if(et.isAlive)
		    {
			   switch(et.direction)
		    	  {
		    	   ...
		    	   case 0:
		    		   /*
		    		    * 下面这句话很重要，当敌人坦克碰着墙以后，isTouchWall的值
		    		    * 变为true,这时敌人坦克无法前进，但之后并没有将isTouchWall的值改为
		    		    * false,所以按任何一个方向，坦克都无法移动，下面这句话的意思就是改
		    		    * isTouchWall的值为false,使敌人坦克可以继续移动。（主要意义是
		    		    * 当敌人坦克是除了“向上”后的其他方向，而且碰着墙了，这时isTouchWall
		    		    * 值为true,按向上时，就可以改isTouchWall为false,使其可以继续移动）
		    		    */
		    		   et.setTouchWall(false);
		    		   for(int j=0;j<4;j++)
		    		   for(int k=0;k<12;k++)
		    		   { //取出墙中的一个基本单元（即墙对象）
		    			 //总共有四堵墙
		    			 ...
		    			 if(wall.isAlive)
		    			 {
		    				if(wall.x>et.x-10&&wall.x<et.x+10&&wall.y+5>et.y-15&&wall.y+5<et.y+15)
		    				{
		    				   et.setTouchWall(true);
		    				 }
		    				
		    				if(wall.x+8>et.x-10&&wall.x+8<et.x+10&&wall.y+5>et.y-15&&wall.y+5<et.y+15)
		    				{
		    					et.setTouchWall(true);
		    				 }
		    			 ...
		    			 if(wall1.isAlive)
		    			 { 
		    				if(wall1.x>et.x-10&&wall1.x<et.x+10&&wall1.y+5>et.y-15&&wall1.y+5<et.y+15)
		    	    	     {
		    					et.setTouchWall(true);	
		    	    		  }
		    				
		    	    		if(wall1.x+8>et.x-10&&wall1.x+8<et.x+10&&wall1.y+5>et.y-15&&wall1.y+5<et.y+15)
		    	    		 {
		    	    			et.setTouchWall(true);
		    	    		  }
		    			 ...
		    			 if(wall2.isAlive)
		    			 {
		    				if(wall2.x>et.x-10&&wall2.x<et.x+10&&wall2.y+5>et.y-15&&wall2.y+5<et.y+15)
		    	    		 {
		    					et.setTouchWall(true);
		    	    		   }
		    				
		    	    		if(wall2.x+8>et.x-10&&wall2.x+8<et.x+10&&wall2.y+5>et.y-15&&wall2.y+5<et.y+15)
		    	    		 {
		    	    			et.setTouchWall(true);
		    	    		   }
		    			 ...
		    			 if(wall3.isAlive)
		    			 {
		    				if(wall3.x>et.x-10&&wall3.x<et.x+10&&wall3.y+5>et.y-15&&wall3.y+5<et.y+15)
		    	    		 {
		    					et.setTouchWall(true);
		    	    		   }
		    				
		    	    		if(wall3.x+8>et.x-10&&wall3.x+8<et.x+10&&wall3.y+5>et.y-15&&wall3.y+5<et.y+15)
		    	    		 {
		    	    			et.setTouchWall(true);
		    	    		   } 
		    	             }
		    	     ...
		    	     case 1:
				    	 ...
				    	 if(wall.isAlive)
				    	  {
				    		 if(wall.x>et.x-10&&wall.x<et.x+10&&wall.y>et.y-15&&wall.y<et.y+15)
				    		 {
				    			 et.setTouchWall(true);	
				    		  }
				    		
				    		  if(wall.x+8>et.x-10&&wall.x+8<et.x+10&&wall.y>et.y-15&&wall.y<et.y+15)
				    		 {
				    			 et.setTouchWall(true);
				    		   }
				    	 ...
				    	 if(wall1.isAlive)
				    	   { 
				    		 if(wall1.x>et.x-10&&wall1.x<et.x+10&&wall1.y>et.y-15&&wall1.y<et.y+15)
				    	     {
				    			 et.setTouchWall(true);
				    	    	}
				    		
				    	     if(wall1.x+8>et.x-10&&wall1.x+8<et.x+10&&wall1.y>et.y-15&&wall1.y<et.y+15)
				    	     {
				    	    	 et.setTouchWall(true);
				    	      }
				    	 ...
				    	 if(wall2.isAlive)
				    		{
				    		 if(wall2.x>et.x-10&&wall2.x<et.x+10&&wall2.y>et.y-15&&wall2.y<et.y+15)
				    		{
				    			 et.setTouchWall(true);	
				    		  }
				    		
				    		 if(wall2.x+8>et.x-10&&wall2.x+8<et.x+10&&wall2.y>et.y-15&&wall2.y<et.y+15)
				    		{
				    			 et.setTouchWall(true);
				    		  }
				    	 ...
				    	 if(wall3.isAlive)
				    	  {
				    		 if(wall3.x>et.x-10&&wall3.x<et.x+10&&wall3.y>et.y-15&&wall3.y<et.y+15)
				    	     {
				    			 et.setTouchWall(true);
				    	      }
				    		
				    	     if(wall3.x+8>et.x-10&&wall3.x+8<et.x+10&&wall3.y>et.y-15&&wall3.y<et.y+15)
				    	     {
				    	    	 et.setTouchWall(true);
				    	      }
		    		 ...
		    		 //坦克向左时
		      	     case 2:
					    ...
					    if(wall.isAlive)
					    {
					    	if(wall.x+8>et.x-15-10&&wall.x+8<et.x+15&&wall.y>et.y-10&&wall.y<et.y+10)
		      				{
					    		et.setTouchWall(true);
		      				 }
					    	
		      				if(wall.x+8>et.x-15&&wall.x+8<et.x+15&&wall.y+5>et.y-10&&wall.y+5<et.y+10)
		      				{
		      					et.setTouchWall(true);
		      				 }
					    ...
					    if(wall1.isAlive)
					    { 
					    	if(wall1.x+8>et.x-15-10&&wall1.x+8<et.x+15&&wall1.y>et.y-10&&wall1.y<et.y+10)
		      				{
					    		et.setTouchWall(true);	
		      				 }
					    	
		      				if(wall1.x+8>et.x-15&&wall1.x+8<et.x+15&&wall1.y+5>et.y-10&&wall1.y+5<et.y+10)
		      				{
		      					et.setTouchWall(true);
		      				 }
					    ...
					    if(wall2.isAlive)
					    {
					    	if(wall2.x+8>et.x-15-10&&wall2.x+8<et.x+15&&wall2.y>et.y-10&&wall2.y<et.y+10)
		      				{
					    		et.setTouchWall(true);	
		      				 }
					    	
		      				if(wall2.x+8>et.x-15&&wall2.x+8<et.x+15&&wall2.y+5>et.y-10&&wall2.y+5<et.y+10)
		      				{
		      					et.setTouchWall(true);	
		      				 }
					    ...
					    if(wall3.isAlive)
					    {
					    	if(wall3.x+8>et.x-15-10&&wall3.x+8<et.x+15&&wall3.y>et.y-10&&wall3.y<et.y+10)
		      				{
					    		et.setTouchWall(true);
		      				 }
					    	
		      				if(wall3.x+8>et.x-15&&wall3.x+8<et.x+15&&wall3.y+5>et.y-10&&wall3.y+5<et.y+10)
		      				{
		      					et.setTouchWall(true);	
		      				 }
		      	     ...
		      	     case 3:
						 ...
						 if(wall.isAlive)
						  {
							if(wall.x>et.x-15&&wall.x<et.x+15&&wall.y>et.y-10&&wall.y<et.y+10)
			      		      {
								et.setTouchWall(true);	
			      			    }
							
			      		    if(wall.x>et.x-15&&wall.x<et.x+15&&wall.y+5>et.y-10&&wall.y+5<et.y+10)
			      		      {
			      		    	et.setTouchWall(true);
			      				}
						 ...
						 if(wall1.isAlive)
						  { 
							 if(wall1.x>et.x-15&&wall1.x<et.x+15&&wall1.y>et.y-10&&wall1.y<et.y+10)
			      				{
								    et.setTouchWall(true);
			      				 }
							
			      				if(wall1.x>et.x-15&&wall1.x<et.x+15&&wall1.y+5>et.y-10&&wall1.y+5<et.y+10)
			      				{
			      					et.setTouchWall(true);	
			      				 }
						 ...
						 if(wall2.isAlive)
						  {
							 if(wall2.x>et.x-15&&wall2.x<et.x+15&&wall2.y>et.y-10&&wall2.y<et.y+10)
			      				{
								    et.setTouchWall(true);	
			      				 }
							 
			      				if(wall2.x>et.x-15&&wall2.x<et.x+15&&wall2.y+5>et.y-10&&wall2.y+5<et.y+10)
			      				{
			      					et.setTouchWall(true);	
			      				 }
						  ...
						  if(wall3.isAlive)
						   {
							  if(wall3.x>et.x-15&&wall3.x<et.x+15&&wall3.y>et.y-10&&wall3.y<et.y+10)
			      				{
								   et.setTouchWall(true);		
			      				 }
							 
			      			  if(wall3.x>et.x-15&&wall3.x<et.x+15&&wall3.y+5>et.y-10&&wall3.y+5<et.y+10)
			      				{
			      				   et.setTouchWall(true);		
			      				 }
		     ...
		     {  //取出我的坦克的子弹
				...
				for(int k=0;k<12;k++)
				{
					 ...
					 //判断我的子弹是否击中墙1中的墙对象
					 if(wall.isAlive){
					 if(myshot.x>wall.x&&myshot.x<wall.x+8&&myshot.y>wall.y&&myshot.y<wall.y+5)
					 {
						 myshot.isAlive=false;
						 wall.isAlive=false;
					  }
					 }
					 //判断我的子弹是否击中墙2中的墙对象
					 if(wall1.isAlive){
					 if(myshot.x>wall1.x&&myshot.x<wall1.x+8&&myshot.y>wall1.y&&myshot.y<wall1.y+5)
					 {
						 myshot.isAlive=false;
						 wall1.isAlive=false;
					  }
					 ...
					 //判断我的子弹是否击中墙3中的墙对象
					 if(wall2.isAlive){
					 if(myshot.x>wall2.x&&myshot.x<wall2.x+8&&myshot.y>wall2.y&&myshot.y<wall2.y+5)
					 {
						 myshot.isAlive=false;
						 wall2.isAlive=false;
					  }
					 ...
					 //判断我的子弹是否击中墙4中的墙对象
					 if(wall3.isAlive){
					 if(myshot.x>wall3.x&&myshot.x<wall3.x+8&&myshot.y>wall3.y&&myshot.y<wall3.y+5)
					 {
						 myshot.isAlive=false;
						 wall3.isAlive=false;
					  }
...
//		    	if(et.isAlive)
//		    	{
		    	for(int r=0;r<et.ss.size();r++)
			     {  
		    		//取出敌人坦克中的子弹
			        Shot myshot=et.ss.get(r);
			        if(myshot.isAlive)
			        {
			        for(int j=0;j<4;j++)
					for(int k=0;k<12;k++)
